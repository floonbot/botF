...
module.exports = async (bot, message) => {
  ...
  db.query(`SELECT logs FROM server WHERE guild = '${message.guild.id}'`, async (err, req) => {
    ...
    try {
      ...
      UNE ERREUR DANS L'EVENT INVITECREATE !!
      ...
      channel.send({ content: `⚠️ UNE ERREUR DANS L'EVENT INVITECREATE !!`, files: [{ attachment: './erreur.txt', name: 'erreur.txt', description: "L'erreur obtenue" }] })
    }
  })
}